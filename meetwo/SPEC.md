...
- 併せて、各サーバーのログファイルの最終更新時刻を取得し、「n秒前/n分前/n時間前/n日前」の形式で表示する(RCONが応答しない場合でも、ログが直近まで更新されていればサーバー自体は動いている可能性がある、といった切り分けに使える)。
- サーバーごとに1つのDiscord Embedを作り、まとめて1つのメッセージとして返信する(1メッセージに複数Embedを含められるため、サーバーごとに色分けできる)。
  - `list`の出力を正規表現(`There are (\d+) of a max of (\d+) players online:?\s*(.*)`)で解析し、「オンライン人数/最大人数」と参加者名を分けて表示する。解析できなかった場合は生の出力をそのまま表示する。
  - 成功時: タイトル `🟢 <サーバー名>`、緑色のEmbed、フィールド「プレイヤー: n/m人」「最終ログ更新: n秒前」(参加者がいれば「参加者: 名前一覧」も追加)。
  - タイムアウトまたは失敗時: タイトル `🔴 <サーバー名>`、赤色のEmbed、フィールド「状態: 応答なし」「最終ログ更新: n秒前」。
